...
def detectPose(image, pose, display=True):
    ...
    print('LEFT_ELBOW',landmark1[mp_pose.PoseLandmark.LEFT_ELBOW.value])
    print('RIGHT_ELBOW',landmark1[mp_pose.PoseLandmark.RIGHT_ELBOW.value])
    print('RIGHT_SHOULDER',landmark1[mp_pose.PoseLandmark.RIGHT_SHOULDER.value])
    print('LEFT_SHOULDER',landmark1[mp_pose.PoseLandmark.LEFT_SHOULDER.value])
    print('RIGHT_KNEE',landmark1[mp_pose.PoseLandmark.RIGHT_KNEE.value])
    print('LEFT_KNEE',landmark1[mp_pose.PoseLandmark.LEFT_KNEE.value])
    print('LEFT_HIP',landmark1[mp_pose.PoseLandmark.LEFT_HIP.value])
    print('RIGHT_HIP',landmark1[mp_pose.PoseLandmark.RIGHT_HIP.value])
    print('LEFT_ANKLE',landmark1[mp_pose.PoseLandmark.LEFT_ANKLE.value])
    print('RIGHT_ANKLE',landmark1[mp_pose.PoseLandmark.RIGHT_ANKLE.value])
...
def calculateAngle(landmark1, landmark2, landmark3):
   ...
   # angle = math.degrees(math.atan2(landmark3[1] - landmark2[1], landmark3[0] - landmark2[0]) - math.atan2(y2 - y1, x2 - x1))
...
def classifyPose(landmarks, output_image, display=False):
    ...
    print("left_elbow_angle:", right_shoulder_angle)
    ...
    print("left_wrist_angle:", left_wrist_angle)
    ...
    print("right_wrist_angle:", right_wrist_angle)
    ...
    # Draw Data
    shapes = np.zeros_like(output_image, np.uint8)

    cv2.rectangle(output_image,(5, 5), (100, 75), (255, 255, 255), cv2.FILLED)
    text_posx = 20;
    text_step = 40
    out = output_image.copy()
    alpha = 0.5
    mask = output_image.astype(bool)
    out[mask] = cv2.addWeighted(output_image, alpha, shapes, 1 - alpha, 0)[mask]
    cv2.putText(output_image, "L_elbow_angle : " + str("{:0.2f}".format(left_elbow_angle)), (10, text_posx),cv2.FONT_HERSHEY_PLAIN, 1.3, (150,75,0), 2)
    cv2.putText(output_image, "R_elbow_angle : " + str("{:0.2f}".format(right_elbow_angle)), (10, text_posx+text_step*1),cv2.FONT_HERSHEY_PLAIN, 1.3, (150,75,0), 2)
    cv2.putText(output_image, "L_shoulder_angle : " + str("{:0.2f}".format(left_shoulder_angle)), (10, text_posx+text_step*2),cv2.FONT_HERSHEY_PLAIN, 1.3, (150,75,0), 2)
    cv2.putText(output_image, "R_shoulder_angle : " + str("{:0.2f}".format(right_shoulder_angle)), (10, text_posx+text_step*3),cv2.FONT_HERSHEY_PLAIN, 1.3, (150,75,0), 2)
    cv2.putText(output_image, "L_wrist_angle : " + str("{:0.2f}".format(left_wrist_angle)), (10, text_posx+text_step*4),cv2.FONT_HERSHEY_PLAIN, 1.3, (150,75,0), 2)
    cv2.putText(output_image, "R_wrist_angle : " + str("{:0.2f}".format(right_wrist_angle)), (10, text_posx+text_step*5),cv2.FONT_HERSHEY_PLAIN, 1.3, (150,75,0), 2)
